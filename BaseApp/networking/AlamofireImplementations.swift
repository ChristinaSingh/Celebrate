...
public class AlamofireRequestBuilder<T>: RequestBuilder<T> {
    ...
    /**
     ...
     configuration.
     */
    public func createSessionManager() -> Alamofire.Session {
     ...
     */
    public func makeRequest(manager: Session, method: HTTPMethod, encoding: ParameterEncoding, headers: [String:String]) -> DataRequest {
            return manager.request(URLString, method: method, parameters: parameters, encoding: encoding, headers: HTTPHeaders(headers))
        }

    override public func execute(isFormData: Bool = false, _ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {
        ...
        if fileKeys.count > 0 || isFormData {
    ...
    public func buildHeaders() -> HTTPHeaders {
...
public class AlamofireDecodableRequestBuilder<T:Codable>: AlamofireRequestBuilder<T> {
        ...
        default:
            validatedRequest.responseData(completionHandler: { (dataResponse: AFDataResponse<Data>) in
                ...
                case let .success(data):
                    ...
                    print(responseObj?.body.convertToString ?? "")
