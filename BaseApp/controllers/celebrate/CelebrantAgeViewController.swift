//
//  CelebrantAgeViewController.swift
//  BaseApp
//
...
class CelebrantAgeViewController: BaseViewController, UITextFieldDelegate {
    
    private let area:Area?
    private let day:Day?
    private let time:TimeSlot?
    private let occasions:[CelebrateOcassion]
    private let celebrantName:String
    
    init(area: Area?, day: Day?, time: TimeSlot?, occasions:[CelebrateOcassion],celebrantName:String) {
        self.area = area
        self.day = day
        self.time = time
    ...
    private lazy var age:C8TextField = {
    ...
    override func setup() {
        ...
        [back , titleLbl , messageLbl , age, proceedBtn].forEach { view in
    ...
    override func actions() {
        proceedBtn.tap = {
            let vc = CelebrateCategoriesViewController(area: self.area, day: self.day, time: self.time, occasions: self.occasions)
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        proceedBtn.enableDisableSaveButton(isEnable: updatedText.trimmingCharacters(in: .whitespaces).count > 0 && Int(updatedText) != 0)
